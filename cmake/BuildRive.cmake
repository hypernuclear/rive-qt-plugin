...
"exit $riveExit\n"
    ...
    # The macOS RIVE_BUILD_CMD scrubs PATH (see further down) so we need
    # to resolve the tools explicitly. Search order mirrors the Windows
    # PowerShell wrapper:
    #   1. Vendored copy under cmake/vendor/<tool>-macos-<arch> (not
    #      shipped today — the search hook is here so dropping a
    #      checked-in binary later requires no further wrapper edits).
    #   2. $VULKAN_SDK/bin (LunarG SDK install).
    #   3. Homebrew prefixes (/opt/homebrew/bin on Apple Silicon,
    #      /usr/local/bin on Intel) — `brew install glslang spirv-tools`.
    #   4. Already on PATH (e.g. user-managed install).
    # Stage resolved binaries with their bare names in a build-local
    # shim dir so the make recipes (which hard-code 'glslangValidator'
    # / 'spirv-opt') find them. The block is only emitted when Vulkan
    # is enabled, so the wrapper stays a no-op in the OFF case.
    if(APPLE AND RIVE_VULKAN_FLAG)
        string(CONCAT _rive_apple_vulkan_block
"# Resolve glslangValidator + spirv-opt for the rive shader build.\n"
"TOOL_SHIM_DIR=\"${RIVE_BUILD_DIR}/tool-shims\"\n"
"mkdir -p \"$TOOL_SHIM_DIR\"\n"
"arch=$(uname -m)\n"
"VENDOR_DIR=\"${CMAKE_CURRENT_LIST_DIR}/vendor\"\n"
"resolve_tool() {\n"
"    local name=$1\n"
"    if [ -x \"$VENDOR_DIR/$name-macos-$arch\" ]; then\n"
"        printf '%s' \"$VENDOR_DIR/$name-macos-$arch\"\n"
"        return\n"
"    fi\n"
"    if [ -n \"$VULKAN_SDK\" ] && [ -x \"$VULKAN_SDK/bin/$name\" ]; then\n"
"        printf '%s' \"$VULKAN_SDK/bin/$name\"\n"
"        return\n"
"    fi\n"
...
"    fi\n"
"}\n"
"glslang=$(resolve_tool glslangValidator)\n"
"spvopt=$(resolve_tool spirv-opt)\n"
"if [ -z \"$glslang\" ] || [ -z \"$spvopt\" ]; then\n"
"    echo \"BuildRive: glslangValidator and/or spirv-opt not found, but --with_vulkan was requested.\" >&2\n"
"    echo \"\" >&2\n"
"    echo \"Install via Homebrew:\" >&2\n"
"    echo \"    brew install glslang spirv-tools\" >&2\n"
"    echo \"\" >&2\n"
"    echo \"Or install the Vulkan SDK from https://vulkan.lunarg.com/sdk/home#mac\" >&2\n"
"    echo \"\" >&2\n"
"    echo \"Or skip the Vulkan backend by re-configuring with:\" >&2\n"
"    echo \"    -DRIVE_QT_WITH_VULKAN=OFF\" >&2\n"
"    exit 1\n"
"fi\n"
"# Stage tools under bare names. The make recipes hard-code\n"
"# 'glslangValidator' / 'spirv-opt'; if we resolved a vendored\n"
"# arch-suffixed copy, the shim hides the suffix. Use symlinks so\n"
"# resolved binaries keep their original @loader_path / rpath\n"
"# context (Homebrew's spirv-opt links libSPIRV-Tools-shared.dylib\n"
"# from /opt/homebrew/lib via an absolute install_name; copying\n"
"# the executable would still work but symlinking is closer to\n"
"# the source-of-truth and avoids future breakage if Homebrew\n"
"# moves to relative paths).\n"
"for entry in \"glslangValidator:$glslang\" \"spirv-opt:$spvopt\"; do\n"
"    bare=\${entry%%:*}\n"
"    src=\${entry#*:}\n"
"    target=\"$TOOL_SHIM_DIR/$bare\"\n"
"    ln -sf \"$src\" \"$target\"\n"
"done\n"
"export PATH=\"$TOOL_SHIM_DIR:$PATH\"\n"
"echo \"BuildRive: using $glslang for SPIR-V compilation\"\n"
"echo \"BuildRive: using $spvopt for SPIR-V optimization\"\n"
        )
    else()
        set(_rive_apple_vulkan_block "")
    endif()
