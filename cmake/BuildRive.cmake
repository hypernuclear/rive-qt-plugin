...
"    # string via string.format ('<%s>%s</%s>'), then calls p.w which\n"
"    # calls string.format AGAIN. The second call exists purely to\n"
"    # collapse %% -> % (for MSBuild's metadata markers like\n"
"    # `%(PreprocessorDefinitions)` which premake emits as `%%(…)` in\n"
"    # the value), but it crashes when the built string contains any\n"
"    # OTHER % sequence that isn't a valid format specifier. Fix: when\n"
"    # there are no format args, do the %% -> % collapse manually via\n"
"    # string.gsub (no format interpretation) and write directly.\n"
"    # Single-quoted PS strings so PS doesn't eat backslashes.\n"
...
"    # Normalize (%% -> %) then re-escape (% -> %%). p.w's internal\n"
"    # string.format collapses the re-escape back to single % -- same\n"
"    # output as the original code in the success case, but never\n"
"    # crashes on stray % chars. Double-paren on the chained gsub so\n"
"    # only its first return value (the string) is passed to p.w.\n"
...
"    # We tolerate a non-zero exit here: 8/9 sub-projects build fine,\n"
"    # but premake-core's own Premake5 project has a separate vcxproj\n"
"    # command-line mangling bug on VS 17.14 (scripts.c compile gets\n"
"    # cl.exe's whole argv passed as a single filename). We don't need\n"
"    # the final premake5.exe — premake_bootstrap.exe already has our\n"
"    # patched Lua scripts embedded and is a fully functional premake.\n"
...
"\n"
"    # Pick whichever premake was produced: prefer the MSBuild'd one\n"
"    # in bin/release/, fall back to the bootstrap exe.\n"
"    $binRelease = Join-Path $premakeCore 'bin\\release'\n"
"    $releaseExe = Join-Path $binRelease 'premake5.exe'\n"
"    $bootstrapOutExe = Join-Path $premakeCore 'build\\bootstrap\\premake_bootstrap.exe'\n"
"    if (Test-Path $releaseExe) {\n"
"        $sourceExe = $releaseExe\n"
"    } elseif (Test-Path $bootstrapOutExe) {\n"
"        $sourceExe = $bootstrapOutExe\n"
"        Write-Host 'Using premake_bootstrap.exe as premake5 (Premake5 MSBuild step failed, but bootstrap has patched scripts embedded).'\n"
"    } else {\n"
"        Write-Error \"Bootstrap.bat exited $bootstrapExit and produced no usable premake binary.\"\n"
"        exit 1\n"
"    }\n"
"\n"
"    # Install to rive's expected path. Both names so rive's subsequent\n"
"    # `[ -f premake5 ]` check passes without re-bootstrapping.\n"
"    New-Item -ItemType Directory -Force -Path '${_rive_premake_install_native}' | Out-Null\n"
"    Copy-Item -Force $sourceExe (Join-Path '${_rive_premake_install_native}' 'premake5.exe')\n"
"    Copy-Item -Force $sourceExe (Join-Path '${_rive_premake_install_native}' 'premake5')\n"
