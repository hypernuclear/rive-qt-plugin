...
//   - react to reported events and state transitions
//
// `advance()` is NOT Q_INVOKABLE — RiveView drives frame stepping. Every
// frame it also drains `reportedEventAt` into eventReported signals and
// queries `stateChangedByIndex` into stateChanged signals.

#include "rive_event.h"
