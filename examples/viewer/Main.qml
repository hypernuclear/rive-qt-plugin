...
// Generated by qt_add_rive_assets() in CMakeLists.txt for the bundled
// data_binding_test.riv. The singleton element name (`Data_binding_test`)
// matches the .riv basename with first letter capitalized; properties
// inside use lowerCamelCase to satisfy QML's naming rules.
import RiveViewer.Assets
...
ApplicationWindow {
    ...
    // Demonstrates the qt_add_rive_assets() codegen output: at startup
    // log the artboard / SM / VM trigger names that the generator
    // pulled from data_binding_test.riv. These constants are static
    // strings — typing `…onFormSubmitt` (extra t) here would be a
    // compile-time error rather than a silent runtime no-op.
    //
    // The same singleton powers the `tryDataBindingExample()` helper
    // below, which the user can invoke from the View Model tab when
    // data_binding_test.riv is the active sample.
    Component.onCompleted: {
        console.log("[codegen] artboard:", Data_binding_test.artboards.artboard)
        console.log("[codegen] state machine:",
                    Data_binding_test.artboard.stateMachines.state_Machine_1)
        console.log("[codegen] VM triggers:",
                    Data_binding_test.viewModels.personViewModel.triggers.onFormSubmit,
                    "/",
                    Data_binding_test.viewModels.personViewModel.triggers.onFormReset)
    }

    // Fire the form-submit trigger on the bound view model using
    // codegen constants instead of string literals. Returns a small
    // status string for display so the demo surfaces success/failure
    // visibly.
    function tryDataBindingExample() {
        if (!rive.viewModel) return "(no VM bound)"
        const triggerName = Data_binding_test.viewModels.personViewModel.triggers.onFormSubmit
        const t = rive.viewModel.trigger(triggerName)
        if (!t) return "trigger '" + triggerName + "' not found"
        t.fire()
        return "fired " + triggerName
    }
    ...
    ColumnLayout {
        ...
        GridLayout {
            ...
            // Codegen demo — only meaningful when data_binding_test.riv
            // is the active sample (that's the .riv qt_add_rive_assets
            // was invoked on in the viewer's CMakeLists.txt).
            Button {
                text: qsTr("Codegen: fire onFormSubmit")
                onClicked: status.text = window.tryDataBindingExample()
                ToolTip.text: qsTr("Calls vmi.trigger(...).fire() using a name " +
                                   "looked up via the auto-generated " +
                                   "RiveViewer.Assets singleton — typo-proof at " +
                                   "compile time. Pick data_binding_test.riv to " +
                                   "see it actually fire.")
                ToolTip.visible: hovered
                ToolTip.delay: 600
            }
